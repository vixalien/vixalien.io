...
const Index = ({ blogs }) => {
  ...
  let searchpars = (key, value) => {
    ...
    try {
      ...
      if (!value) { searchObj.delete(key) }
      if (key && value) { searchObj.set(key, value); }
      let str = searchObj.toString();
        history.replaceState({ searched_for: value }, null, new URL((str.length > 0?("?" + str):location.pathname), location));
  ...
  let filter = ({ target: bar }) => {
    if (!bar.value) { setBlogs(blogs); searchpars("q"); return };
